...
def load_and_process_data(ticker, start_date, end_date, local_file=None, split_ratio=0.7, split_by_date=False, columns_to_scale=None):
    ...
    # Add Google Trends data (Task 7 - Extension) (If we dont want use it - comment to disable the code)
...
if __name__ == "__main__":
    ...
    # Load trend data separately and display it (Task B.7 - Extension)
    trends_cache_file = f"/content/drive/My Drive/Cos30018/{ticker}_trends.csv"
    trends_data = pd.read_csv(trends_cache_file, index_col='date', parse_dates=True)
    display_trend_data(trends_data)

    # Task B.3 - Data Processing 2 (code to run)
    ...
    # Task B.4 - Machine learning 1 (code to run)
    ...
    # Task B.5 - Machine learning 2 (code to run)
    ...
    # Task B.6 - Machine Learning 3 (Code to run) (Try to comment task B.7 - Extension to run it)
    ...
    plt.title('Stock Price Prediction: LTSM vs ARIMA vs Ensemble')
    plt.xlabel('Time')
    plt.ylabel('Stock Price')
    plt.legend()
    plt.show()
    ...
    # Task B.7 - Extension (Code to run) (Try to comment task B.6 - Machine Learning 3 to run it)
